                ...
                <>
                    {props.subjects.map(subject => (
                        <div className="lessonDashboard__card" key={subject["id"]}>
                            <button
                                className="lessonDashboard__card-deleteBtn"
                                onClick={onDeleteSubject.bind(null, subject["id"])}
                            >
                                &times;
                            </button>
                            <br></br>
                            <div
                                className="lessonDashboard__card-container"
                                onClick={loadSubject.bind(this, subject)}
                            >
                                <div className="lesson-img">
                                    <img
                                        src={
                                            "https://unsplash.it/2000/10" + subject["id"]
                                        }
                                    ></img>
                                </div>
                                <h4>
                                ...
                                <p>
                                    <strong>
                                        Total Lessons: {subject["lessons"].length}
                                    </strong>
                                </p>
    ...
    return (
        <div className="lessonDashboard__div-Area">
            <div className="lessonDashboard__div-Side">
                <SidePanel
                    subjectHandler={onSubjectHandler.bind(this)}
            ...
            <div className="lessonDashboard__div-Main">
                ...
                {loadLessonForm && (
                    <Backdrop formHandler={onLessonFormHandler.bind(null)} />
                )}
                {loadLessonForm && (
                    <LessonForm formHandler={onLessonFormHandler.bind(null)} />
                )}

                {loadVideoForm && (
                    <Backdrop formHandler={onVideoFormHandler.bind(this)} />
                )}
                {loadVideoForm && (
                    <LessonVideoForm formHandler={onVideoFormHandler.bind(this)} />
                )}
...
export default React.memo(
    connect(
        mapStateToProps,
        { getSubjects, deleteSubject, addSubjectInfo }
    )(SubjectDashboard)
);
